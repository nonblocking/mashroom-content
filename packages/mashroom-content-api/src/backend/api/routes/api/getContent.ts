...
export default async (req: Request, res: Response) => {
    ...
    const { type: contentType, id } = req.params as paths['/content/api/{type}/{id}']['get']['parameters']['path'];
    const { locale, version: versionStr } = req.query as paths['/content/api/{type}/{id}']['get']['parameters']['query'];

    try {
        const version = versionStr ? parseInt(versionStr) : undefined;
        const result = await contentService.getContent<any>(req, contentType, id, locale, version);
        if (!result) {
            res.sendStatus(404);
            return;
        }

        const response: paths['/content/api/{type}/{id}']['get']['responses']['200']['content']['application/json'] = result;
